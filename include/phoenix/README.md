...
从零开始重写[phoenix C++高性能分布式服务器框架](https://github.com/phoenix-yin/phoenix)。

在线文档：[从零开始重写phoenix C++高性能分布式服务器框架](https://www.midlane.top/wiki/pages/viewpage.action?pageId=10060952)。
...
```cpp
phoenix_LOG_INFO(g_logger) << "this is a log";
```

格式化日志使用：

```cpp
phoenix_LOG_FMT_INFO(g_logger, "%s", "this is a log"); 
```
...
本项目的日志模块基于phoenix的进行了简化，同时参考了log4cpp的一些设计，在保证功能可用的情况下，简化了几个类的设计，降低了耦合度。目前来看，在LogAppender上仍然需要丰富，以下几种类型的Appender在实际项目中都非常有必要实现：
...
工具接口与工具类，功能宏定义。包括获取时间，日期时间格式转换，栈回溯，文件系统操作接口，类型转换接口，以及phoenix_ASSERT宏。详细接口参考util.h，macro.h。
...
phoenix对以下函数进行了hook，并且只对socket fd进行了hook，如果操作的不是socket fd，那会直接调用系统原本的api，而不是hook之后的api：  
