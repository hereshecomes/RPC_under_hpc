...
IOManager::FdContext::EventContext &IOManager::FdContext::getEventContext(IOManager::Event event) {
    ...
    case IOManager::WRITE:
        return write;
    default:
        phoenix_ASSERT2(false, "getContext");
    }
...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (phoenix_UNLIKELY(fd_ctx->events & event)) {
        phoenix_LOG_ERROR(g_logger) << "addEvent assert fd=" << fd
                                  << " event=" << (EPOLL_EVENTS)event
                                  << " fd_ctx.event=" << (EPOLL_EVENTS)fd_ctx->events;
        phoenix_ASSERT(!(fd_ctx->events & event));
    }
    ...
    if (rt) {
        phoenix_LOG_ERROR(g_logger) << "epoll_ctl(" << m_epfd << ", "
    ...
    phoenix_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        phoenix_ASSERT2(event_ctx.fiber->getState() == Fiber::RUNNING, "state=" << event_ctx.fiber->getState());
    }
    return 0;
}
...
bool IOManager::cancelAll(int fd) {
    ...
    phoenix_ASSERT(fd_ctx->events == 0);
    return true;
}
...
void IOManager::tickle() {
    phoenix_LOG_DEBUG(g_logger) << "tickle";
    if(!hasIdleThreads()) {
        return;
    }
...
void IOManager::idle() {
    ...
    while (true) {
        ...
        if( phoenix_UNLIKELY(stopping(next_timeout))) {
            phoenix_LOG_DEBUG(g_logger) << "name=" << getName() << "idle stopping exit";
            break;
        }
