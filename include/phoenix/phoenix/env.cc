    ...
    for (int i = 1; i < argc; ++i) {
        if (argv[i][0] == '-') {
            if (strlen(argv[i]) > 1) {
                ...
                phoenix_LOG_ERROR(g_logger) << "invalid arg idx=" << i
                                          << " val=" << argv[i];
                return false;
            }
        } else {
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
    ...
    static phoenix::ConfigVar<std::string>::ptr g_server_work_path =
        phoenix::Config::Lookup<std::string>("server.work_path");
